...
import {
  ...
  AppErrStatus,
  AppResult,
} from "@application/utils/app-result.utils";
...
describe("AppResult", () => {
  describe("static constructors", () => {
    it("should create Ok result", () => {
      const result = AppResult.Ok("success");
    ...
    it("should create Err result", () => {
      ...
      const result = AppResult.Err(error);
    ...
    it("should create result from Result type", () => {
      const okResult = AppResult.fromResult(Result.Ok("test"));
      const errResult = AppResult.fromResult(Result.Err(new Error("fail")));
  ...
  describe("transformations", () => {
    ...
    it("should not transform Err values", () => {
      const result = AppResult.Err(new Error("test")).map((x) => x * 2);
    ...
    it("should flatMap correctly", () => {
      const result = AppResult.Ok(5).flatMap((x) => Result.Ok(x * 2));
    ...
    it("should handle flatMap errors", () => {
      const result = AppResult.Ok(5).flatMap((_x) =>
  ...
  describe("AppError", () => {
    it("should create different error types", () => {
      ...
      expect(AppError.InternalError("").status).toBe(
    ...
    it("should map different error types using instanceof checks", () => {
      ...
      expect(AppError.fromErr(notFoundError).status).toBe(
        AppErrStatus.NotFound,
      );
      ...
      expect(AppError.fromErr(conflictError).status).toBe(
        AppErrStatus.Conflict,
      );
