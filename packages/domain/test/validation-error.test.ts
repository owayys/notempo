...
import { ThoughtCreateSchema } from "@domain/thought/thought.entity";
import { ValidationError } from "@domain/utils/base.errors";
import {
  validationErrorsToSingle,
  zodErrorsToValidationError,
...
describe("Validation Error Utils", () => {
  describe("parseErrorsToValidationError", () => {
    it("should handle empty array", () => {
      const result = zodErrorsToValidationError([]);
    ...
    it("should handle real parse error from empty string validation", () => {
      ...
      const result = z.safeDecode(ThoughtCreateSchema, {
        text: "", // Invalid: empty string (violates minLength(1))
        authorId: "Invalid ID", // Invalid: not a UUID
      });

      expect(result.error).toBeDefined();

      if (result.error) {
        const validationResult = zodErrorsToValidationError([result.error]);
    ...
    it("should handle real parse error from missing required fields", () => {
      ...
      const result = z.safeDecode(ThoughtCreateSchema, {
        text: "Valid Text",
        // Invalid: missing required authorId field
      });
    ...
    it("should handle real parse error from wrong data types", () => {
      ...
      const result = z.safeDecode(ThoughtCreateSchema, {
        ...
        // @ts-expect-error Testing invalid input
        authorId: true, // Invalid: should be UUID string
      });
    ...
    it("should handle real parse error from null input", () => {
      ...
      // @ts-expect-error Testing invalid input
      const result = z.safeDecode(ThoughtCreateSchema, null);
    ...
    it("should handle multiple real parse errors", () => {
      ...
      const result = z.safeDecode(TestSchema, {
